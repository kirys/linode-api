...
public class Linode {
     ...
     *
     * @param action API action to execute
     ...
     * request.
     *
     * @param requests array of LinodeRequest
    ...
    public Object batchExecute(List<LinodeRequest> requests) throws IOException, HttpException, JSONException {
        ...
        nvps.add(new BasicNameValuePair("api_requestArray", params.toString()));
        ...
        httpResponse.getEntity().writeTo(bas);
        String response =bas.toString("UTF8");
        if (debug) {
     ...
     *
     * @param action API action to invoke
     ...
     *
     * @param b StringBuilder to append to
     * @param parameters parameters to append in key-value format
